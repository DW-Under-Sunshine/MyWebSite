...
@interface DWLeftSlideControllerOne () <LeftViewControllerDelegate>

@property (nonatomic, strong) UIViewController *mainVC;
@property (nonatomic, strong) LeftViewController *leftVC;
@property (nonatomic, strong) HostSiteViewController *hostPage;
@property (nonatomic, strong) WeddingPhotoViewController *webPage;
@property (nonatomic, assign) CGFloat PanGestureNowX;
@end

@implementation DWLeftSlideControllerOne
-(HostSiteViewController *)hostPage
{
    if (_hostPage == nil) {
        _hostPage = [[HostSiteViewController alloc]init];
    }
    return _hostPage;
}
-(WeddingPhotoViewController *)webPage
{
    if (_webPage == nil) {
        _webPage = [[WeddingPhotoViewController alloc]init];
    }
    return _webPage;
}
...
- (void)LeftViewControllerdidSelectRow:(LeftViewControllerRowType)LeftViewControllerRowType{
    ...
    if (LeftViewControllerRowType == LeftViewControllerRowTypeOne) {
        ...
        [nav pushViewController:self.hostPage animated:NO];
    }else if (LeftViewControllerRowType == LeftViewControllerRowTypeFour){
        NSLog(@"LeftViewControllerRowTypeFour");
        UINavigationController *nav = (UINavigationController *)_mainVC;
        nav.navigationBar.hidden = YES;
        [nav popToRootViewControllerAnimated:NO];
        [nav pushViewController:self.webPage animated:NO];
//        nav.toolbar.backgroundColor = [UIColor grayColor];
    }else if (LeftViewControllerRowType == LeftViewControllerRowTypeFive){
        NSLog(@"LeftViewControllerRowTypeFive");
    }
